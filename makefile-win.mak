...
kernel: kernel.s functions.asm 
	${ASM} ${ASMFLAGS} kernel.asm -o kernel.o -l kernel.lst
	#gcc ${CFLAGS} -S cstart.c -o cstart.s
	${ASM}  ${ASMFLAGS}  functions.asm -o functions.o > functions.lst
	# ${ASM} -f elf64 functions.asm -o functions.o -l functions.lst
#	gcc ${CFLAGS} -c function.c -o function.o
	#ld --oformat binary -N -e kernel_entry -Ttext 0x7e00 kernel.o cstart.o functions.o -o kernel
	${LD} --oformat binary  -e kernel_entry -Ttext 0x7e00 kernel.o  functions.o -o kernel
	#ld -N -e kernel_entry -Ttext 0x7e00 kernel.o -o kernel.elf
	#objcopy -O binary -S kernel.elf kernel
